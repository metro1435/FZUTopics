    ...
    <el-table-column type="expand">
      <template slot-scope="props">
        <el-form label-position="left" inline class="demo-table-expand">
          ...
          <el-form-item label="评论对象 ID">
            <span>{{ props.row.commentitemid }}</span>
          </el-form-item>
  ...
  export default {
    ...
    methods: {
      handlePass(index, row) {
        ...
        request({
        url: "/admin/ctcomment/unaudited?ctcommentid="+row.commentid+"&auditstatus=1",
        method: "put",
        headers: {
      ...
      handleFail(index, row) {
        console.log(index, row);
        request({
        url: "/admin/ctcomment/unaudited?ctcommentid="+row.commentid+"&auditstatus=0",
        method: "put",
        headers: {
          token: this.$store.state.token
        }
